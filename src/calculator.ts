...
function findCostEntryForDate(costHistory: CostHistoryItems[], targetDate: Date): CostEntryResult {
  ...
  const sortedCostHistory = validCostHistory.sort(
  ...
  if (sortedCostHistory.length === 0) {
    return {
      ok: false,
      message: `no cost data available for this target data (${targetDate.toISOString().split('T')[0]})`
    }
  }
...
function calculateEmployeeCost(employee: Employee, date: Date): EmployeeCostResult {
  ...
  const finalHourlyCost = applyMultipliers(
    baseHourlyRate,
    constEntry.benefitsPercent,
    constEntry.overheadMultiplier
  )
...
function processAllTimeEntries(
  employees: EmployeesFile,
  timeEntries: TimeEntryFile
): ProcessedTimeEntry[] {
